...
- (void)setBackgroundView:(UIView *)iBackgroundView
{
    if (_backgroundView != iBackgroundView) {
        _backgroundView = iBackgroundView;
...
- (void)AwesomeMenuItemTouchesBegan:(AwesomeMenuItem *)item
{
    ...
    {
        if ([_delegate respondsToSelector:@selector(awesomeMenuDidTapOnTheStartItem:)]) {
            [_delegate awesomeMenuDidTapOnTheStartItem:self];
        }
        self.expanding = !self.isExpanding;
    }
}
