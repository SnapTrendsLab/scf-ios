...
-(IBAction)CancelButtonPressed:(id)sender
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WIAVPlayerShowIfNeeded object:nil];
...
- (void)showView
{
    UIImageView *loadingImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"app bg"]];
    [loadingImageView setFrame:self.webView.bounds];
    [self.webView addSubview:loadingImageView];
    [loadingImageView release];
    [WIUtility startActivityIndicatorOnView:self.view withText:@"Loading..." BlockUI:YES];
...
- (void)viewWillAppear:(BOOL)animated {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showView) name:@"kGTMOAuth2UserSignedIn" object:nil];
  if (!isViewShown_) {
    isViewShown_ = YES;
  ...
  [(WIAppDelegate *)[[UIApplication sharedApplication] delegate] hideCamaeraTab];
...
- (void)viewWillDisappear:(BOOL)animated {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"kGTMOAuth2UserSignedIn" object:nil];
  if (!didDismissSelf_) {
